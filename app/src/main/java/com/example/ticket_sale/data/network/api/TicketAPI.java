...
public interface TicketAPI {
    ...
    @GET(BASE_PATH + "/{id}")
    Call<ApiResponse<TicketDTO>> getTicketById(@Path("id") String id);

    @GET(BASE_PATH + "/get/active")
    Call<ApiResponse<List<TicketDTO>>> getAllActiveTickets();

    //http://localhost:8888/payment-service/api/ticket/all?limit=10&page=0&active=none&orderBy=price&asc=asc
    @GET(BASE_PATH + "/all")
    Call<ApiResponse<PageResponse<TicketDTO>>> getTicketsFilter(
